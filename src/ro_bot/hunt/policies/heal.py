...
Presses the configured heal key when current HP / HP_max falls below
``threshold_pct``. Cooldown-limited so recoveries between presses
don't double-fire.

HP is sourced from the sniffer's 0x00B0 ZC_PAR_CHANGE cache, which is
...
If the sniffer hasn't seen any HP packets yet (``hp_max == 0``) we
explicitly skip; logging a warning periodically so a stuck sniffer
can't silently leave the character undefended.
"""
...
class HealPolicy:
    ...
    def _log_snapshot(self, now: float) -> None:
        ...
        if empty and (now - self._last_empty_warn_at
                      >= EMPTY_HP_CACHE_WARN_INTERVAL_SEC):
            logger.warning(
                "Heal disabled: sniffer HP cache is empty (hp_max=0) — "
                "no 0x00B0 packets seen yet; heal will not fire",
            )
        ...
        if empty:
            logger.info(
                "HP snapshot: hp=0/0 (cache empty) map=%s allowed=%s → skip",
                map_name, allowed,
            )
            return

        ratio_pct = 100.0 * hp / hp_max
        reason = self._skip_reason(hp, hp_max, allowed, threshold_pct)
        logger.info(
